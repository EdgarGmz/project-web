          ...
          {recentSales.length > 0 ? (
            <div className="space-y-4">
              {recentSales.slice(0, 5).map((sale, index) => (
                <div key={sale.id} className="flex items-center justify-between p-3 bg-surface/50 rounded-lg hover:bg-surface/70 transition">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="h-10 w-10 bg-accent/20 rounded-lg flex items-center justify-center flex-shrink-0">
                      <span className="text-accent text-xs font-bold">#{sale.sale_number}</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="font-medium truncate">{sale.customer_name || 'Cliente general'}</div>
                      <div className="text-sm text-muted">
                        {new Date(sale.created_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })} • {sale.total_items} {sale.total_items === 1 ? 'item' : 'items'}
                      </div>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0 ml-4">
                    <div className="font-semibold whitespace-nowrap">{formatCurrency(sale.total)}</div>
                    <div className="text-xs text-muted capitalize">{sale.payment_method}</div>
                  </div>
                </div>
              ))}
            </div>
