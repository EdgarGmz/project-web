 ...
 *         email: { type: string, format: email, description: Email único del usuario, example: "juan.perez@empresa.com" }
 ...
 *         role: { type: string, enum: [admin, manager, cashier], description: Rol del usuario, example: "cashier" }
 *         employee_id: { type: string, description: ID único del empleado, example: "EMP001" }
 *         phone: { type: string, description: Teléfono del usuario, example: "81-1234-5678" }
 *         hire_date: { type: string, format: date, description: Fecha de contratación, example: "2024-01-15" }
 *         branch_id: { type: integer, description: ID de la sucursal asignada, example: 1 }
 *         is_active: { type: boolean, description: Estado activo, example: true }
 *         last_login: { type: string, format: date-time, description: Última fecha de login, example: "2024-10-06T10:30:00Z" }
 *         created_at: { type: string, format: date-time, description: Fecha de creación, example: "2024-10-01T08:00:00Z" }
 *         updated_at: { type: string, format: date-time, description: Fecha de última actualización, example: "2024-10-06T10:30:00Z" }
 *         branch: { $ref: '#/components/schemas/Branch' }
 *     UserInput:
 *       type: object
 *       required: [first_name, last_name, email, password]
 *       properties:
 *         first_name: { type: string, minLength: 2, maxLength: 100, example: "Juan" }
 *         last_name: { type: string, minLength: 2, maxLength: 100, example: "Pérez" }
 *         email: { type: string, format: email, example: "juan.perez@empresa.com" }
 *         password: { type: string, minLength: 6, example: "password123" }
 *         role: { type: string, enum: [admin, manager, cashier], default: "cashier", example: "cashier" }
 *         employee_id: { type: string, maxLength: 20, example: "EMP001" }
 *         phone: { type: string, maxLength: 20, example: "81-1234-5678" }
 ...
 *       type: object
 *       properties:
 *         first_name: { type: string, example: "Juan Carlos" }
 *         last_name: { type: string, example: "Pérez González" }
 *         email: { type: string, format: email, example: "juan.carlos@empresa.com" }
 *         role: { type: string, enum: [admin, manager, cashier], example: "manager" }
 *         employee_id: { type: string, example: "EMP001" }
 *         phone: { type: string, example: "81-9876-5432" }
 *         hire_date: { type: string, format: date, example: "2024-01-15" }
 ...
 *               value: { role: "manager", branch_id: 3 }
 *             contact_update:
 *               summary: Actualizar contacto
 *               value: { phone: "81-5555-6666", email: "nuevo.email@empresa.com" }
 *     responses:
 *       200:
